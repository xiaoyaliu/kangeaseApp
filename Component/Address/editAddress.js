...
export default class LaunchImage extends Component{
	  ...
	  componentDidMount(){
			this.getInitAddress(this.props.id);
			this.setAddress();
	  }
	  getInitAddress(id){
			var self=this;
			let formData = new FormData();
			formData.append("act","getAddressInfo");
	  ...
	  submitAddress(){
			...
			let formData = new FormData();
			formData.append("act", "editAddressInfo");
			formData.append("param1",this.state.is_default);
			...
			formData.append("param9", this.props.id+"");
			Util.get(formData, function (data) {
				  console.log(data)
				  if (data.flag) {
